...
| **H7** — RBAC site-scoping timing left as silent default | HIGH | **EXPLICITLY BLOCKED WITH DOCUMENTED REASON** | §32, §49 | §32a | No architecture change — the finding was that a decision needed to be made explicitly, not that a mechanism was missing (the reserved `RoleAssignment.scope_type` already existed) | None | None | Directly relevant — determines whether every authenticated user can read/write every site's data at Phase 1 launch | **Phase 1 entry criterion — blocking** | **MANAGEMENT DECISION REQUIRED** — Option A/B/C per §32a; Option B noted as the lower-cost default absent a conflicting need, not selected by this document |
...
**On H7's disposition:** per the task's own framework, C1–C5 must be "RESOLVED or EXPLICITLY BLOCKED WITH DOCUMENTED REASON." H7 is not a C-series finding, but the same discipline applies: it is recorded as **explicitly blocked** — the architecture provides the mechanism (`RoleAssignment` scoping) and the decision framework (§32a's three options), but the decision itself is a management action this document cannot and does not take.
...
## Red-Team Finding Disposition (C1–C5, plus H1/H6/H7/M1)

| Finding | Original Severity | Confirmed? | Resolved in v1.2? | Deferred? | Blocking Phase 1? | Reason |
|---|---|---|---|---|---|---|
| C1 | CRITICAL | Confirmed | **RESOLVED** | No | No (was blocking pre-v1.2; resolved) | `ReplaceAsset` operation + `replaces_asset_id` close the gap |
| C2 | CRITICAL | Confirmed | **RESOLVED** | No | No | Dual partial exclusion constraints verified against the full truth table |
...
| H7 | HIGH | Confirmed | **EXPLICITLY BLOCKED WITH DOCUMENTED REASON** | No | **Yes — Phase 1 entry criterion** | Mechanism exists; the choice among Option A/B/C is a management decision this document cannot make |
