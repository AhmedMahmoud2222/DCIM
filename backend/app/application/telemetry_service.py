...
async def ingest_reading(
    db: AsyncSession, *, collector_id: uuid.UUID, integration_id: uuid.UUID,
    dedup_key: str, external_identifier: str, source_identifier: str,
    occurred_at: datetime, value: float, attributes: dict | None = None,
) -> TelemetryIngestResult:
    mapping = (await db.execute(select(IntegrationMetricMapping).where(
    ...
    statement = insert(TelemetryReading).values(
        id=uuid.uuid4(), collector_id=collector_id, integration_id=integration_id, mapping_id=mapping.id,
        external_identifier=external_identifier, dedup_key=dedup_key, metric=mapping.canonical_metric,
        unit=mapping.unit, value=value * float(mapping.scale), occurred_at=occurred_at,
